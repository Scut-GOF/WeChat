...
public class TestData {
	...
	public static List<IdObj> makeContacts() {
		...
		int longNote = 0;
		int nullNote = 0;
		// 生成750条联系人数据，包括姓名，地址，备注
		for (int i = 0; i < 750; i++) {
			// 姓名包括中文，英文，特殊字符（比如颜文字）","，包括火警，10086等特殊电话
			// 地址可长可短可空，50%空，30%短，20%长，长地址从国到房号，短地址可以只有C12-434
			// 备注50%空，40%短，10%超长（比如一段歌词）","
			// 地址空的部分，备注不一定空
			String name = "";
			String lPinYin;
			String sPinYin;
	...
	public static List<IdTelObj> makeIdTels() {
		...
		// ID必须在contacts中存在
		// tel有的是电话，有的是手机，有的带区号，有的是10086等特殊号码
		return idTels;
	}
	...
	public static List<IdLabelObj> makeIdLabels() {
		...
		// 只有40%的联系人是有Label的
		// 在这40%的联系人中
		// 50%有1个label，20%有2个label，20%有3个label，10%有4-20个label
		for (int i = 1; i < 751; i++) {
...
//                    label = labels.get((int)(Math.random() * 19));
	...
	public static void insertData(Context context) {
		List<IdObj> cons = makeContacts();
		List<IdTelObj> idTels = makeIdTels();
		List<String> labels = makeLabels();
		List<IdLabelObj> idLabel = makeIdLabels();

		MainTableUtils mainTableUtils = new MainTableUtils(context);
		TelTableUtils telTableUtils = new TelTableUtils(context);
		LabelTableUtils labelTableUtils = new LabelTableUtils(context);
		IDLabelTableUtils idLabelTableUtils = new IDLabelTableUtils(context);

		for (int i = 0; i < cons.size(); i++) {
			mainTableUtils.insertAll(cons.get(i).getName(), cons.get(i).getAddress(), cons.get(i).getNotes());
			//Log.d("TESTDATA",cons.get(i).getName()+" "+cons.get(i).getAddress()+" "+cons.get(i).getNotes());
		}
		for (int i = 0; i < labels.size(); i++) {
			labelTableUtils.insertAll(labels.get(i), "");
//	        Log.d("TESTDATA",labels.get(i));
		}
		for (int i = 0; i < idLabel.size(); i++) {
			idLabelTableUtils.insertAll(idLabel.get(i).getId(), idLabel.get(i).getLabel());
//	        Log.d("TESTDATA",idLabel.get(i).getId() + " " + idLabel.get(i).getLabel());
		}
		for (int i = 0; i < idTels.size(); i++) {
			telTableUtils.insertAll(idTels.get(i).getId(), idTels.get(i).getTel());
//		    Log.d("TESTDATA",idTels.get(i).getId() + " " + idTels.get(i).getTel());
		}
	}
}